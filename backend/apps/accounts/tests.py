...
class AdminSellerTrendsTests(TestCase):
    ...
    def test_admin_seller_trends_endpoints_return_real_aggregates(self):
        ...
        products = self.client.get("/api/v1/admin/seller-trends/products?period=7d")
        revenue_sorted_products = self.client.get("/api/v1/admin/seller-trends/products?period=7d&sort=revenue")
        sellers = self.client.get("/api/v1/admin/seller-trends/sellers?period=7d")
        keywords = self.client.get("/api/v1/admin/seller-trends/keywords?period=7d")
        reels = self.client.get("/api/v1/admin/seller-trends/reels?period=7d")
        ...
        self.assertGreaterEqual(len(products.data), 2)
        self.assertEqual(products.data[0]["name"], "Organic Herbal Tea Blend")
        self.assertEqual(products.data[0]["orders7d"], 7)
        self.assertGreater(products.data[0]["revenue7d"], 170)
        self.assertEqual(products.data[0]["topMarkets"][0]["code"], "pk")
        self.assertEqual(products.data[0]["views_source"], "derived")
        self.assertEqual(revenue_sorted_products.data[0]["name"], "Smart LED Panel Light")

        self.assertEqual(sellers.data[0]["name"], "GreenLeaf Organics")
        self.assertEqual(sellers.data[0]["orders"], 7)
        self.assertEqual(sellers.data[0]["repeatBuyerRate"], 100)
        self.assertEqual(sellers.data[0]["metrics_source"]["return_rate"], "derived_dispute_rate")
        self.assertEqual(len(sellers.data[0]["monthlySales"]), 6)

        self.assertTrue(any(row["keyword"] == "organic" for row in keywords.data))
        self.assertEqual(keywords.data[0]["source_type"], "derived_order_tokens")
        self.assertEqual(reels.data[0]["seller_name"], "GreenLeaf Organics")
        self.assertIn(reels.data[0]["stats_source"], {"detail_config", "derived"})
    ...
    def test_admin_seller_trends_country_filter_and_cache_invalidation(self):
        ...
        self.assertEqual(len(cached_before.data), 1)
        self.assertEqual(cached_before.data[0]["name"], "Organic Herbal Tea Blend")
